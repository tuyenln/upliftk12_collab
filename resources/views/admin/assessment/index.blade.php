               ...
               <div class="col-12 col-sm-12 col-md-12 col-lg-9 mrgn-b-2">
                  <div class="team-detail-content nopaddingtop">
                     <div class="clearfix header-section-content hasbutton">
                        <h3 class="title-content">Manage Assessments</h3>
                         @if(Session::has('message'))
                           <p class="alert {{ Session::get('alert-class', 'alert-info') }}">{{ Session::get('message') }}</p>
                                    @endif
                           <a href="{{ route('admin.assessment.add') }}" class="btn btn-primary pull-right button-header-content">Create Assessment</a>
                     </div>
                                       ...
                                       <table class="table table-bordered table-striped customtable1 actionlistform">
                                          ...
                                          <tbody>
                                             ...
                                             <tr>
                                                ...
                                                <td>{{$aRow->gl()}}</td>
                                                <td>
                                                   <a class="edit" href="{{ route('admin.assessment.edit',$aRow->id) }}"><i class="lni lni-pencil-alt"></i></a>
                                                   <a class="delete" href="javascript:void(0);" onclick="jQuery(this).parent('td').find('#delete-form').submit();"><i class="lni lni-trash"></i>
                                                   </a>
                                                   <form id="delete-form" onsubmit="return confirm('Are you sure to delete?');" action="{{ route('admin.assessment.destroy',$aRow->id) }}" method="post" style="display: none;">
                                                      {{ csrf_field() }}
                                                   </form>
                                                </td>
                                             </tr>
