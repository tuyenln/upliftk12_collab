...
abstract class question_edit_form extends question_wizard_form {
    ...
    protected function definition() {
        require_once('caseApiPage.php');
        require_once('caseApiFun.php');
        ...
        if (!isset($this->question->id)) {
            ...
            //Adding level
            $mform->addElement('select', 'md_level', 'Level', $levelArray);
          
        
             //Adding CASE
            $mform->addElement('select', 'md_case', 'Case', $caseArray);
            $mform->getElement('md_case')->setMultiple(true);
            }
...
if (!empty($this->question->id)) {
            ...
            if($qtype=='multichoice'){
                    ...
                    //Adding level
                    $selecArrL= _selectLevel($this->question->id);
                    $selclev=explode(',',$selecArrL);

                    $selecArr= _selectCase($this->question->id);
                    $selc=explode(',',$selecArr);

                    $mform->addElement('select', 'md_level', 'Level', $levelArray);
                    $mform->getElement('md_level')->setSelected($selclev);

                    //Adding CASE
                    $mform->addElement('select', 'md_case', 'Case', $caseArray);
                    $mform->getElement('md_case')->setMultiple(true);
                    $mform->getElement('md_case')->setSelected($selc); 
