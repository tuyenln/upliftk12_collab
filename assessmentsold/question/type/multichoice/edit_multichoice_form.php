...
class qtype_multichoice_edit_form extends question_edit_form {
    ...
    protected function get_per_answer_fields($mform, $label, $gradeoptions,
            &$repeatedoptions, &$answersoption) {
                ...
                global $DB;
                $arrDis=[];
                $DistratorList = $DB->get_records_sql("SELECT id,name FROM `mdl_distrators` ORDER BY name ASC");
                foreach ($DistratorList as $row) {
                $arrDis[$row->id]=$row->name;
                }
                $repeated = array();
                ...
                $repeated[] = $mform->createElement('select', 'distrcator','Distractor', $arrDis);
                $repeated[] = $mform->createElement('editor', 'feedback',
                get_string('feedback', 'question'), array('rows' => 1), $this->editoroptions);
                $repeatedoptions['answer']['type'] = PARAM_RAW;
                $repeatedoptions['fraction']['default'] = 0;
                $repeatedoptions['distractor']['default'] = 0;
